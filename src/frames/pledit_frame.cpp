...
void PlEditFrame::on_btn_save_clicked (Control* ctl) {
    ...
    if (info.name.length () > 0) {
        ...
        case 0:
            PlInfos::add (info);
            cb_select->insert_item (PlInfos::players_count () + 1, info.name,
                    trans (info.color));
            cb_select->set_selected (PlInfos::players_count () + 1);
            break;
        case 1:
            PlInfos::add (info);
            cb_select->add_item (info.name, trans (info.color));
...
void PlEditFrame::on_cb_select_changed (Combobox* ctl, int value) {
    if (value != -1) {
        ...
        case 1:
            ...
            break;
        default:
            const PlInfo& info = PlInfos::at(value - 2);
            pl_type = info.type;
