					...
					case SDL_BUTTON_LEFT:
						if (order_col != col) {
							...
							for (plid_tu plid = 0; plid < players.count(); plid++) {
								...
								for (plid_tu other = 0; other < players.count(); other++) {
									int cmp = compare_pl(plid, other, order_col); 
									if (cmp < 0) {								
										pl_orders[plid]++;
									} else if (cmp == 0) {
										if (old_order > players[other].get_order()) {
											pl_orders[plid]++;
										}
									}
								}
							...
							for (plid_tu plid = 0; plid < players.count(); plid++) {
								...
								pl.set_order(players.count() - pl.get_order() - 1);
							}
							for (plid_tu tid = 0; tid < teams.count(); tid++) {
								Team& te = teams[tid];
